...
export default {
  ...
  methods: {
    ...
    // 初始化搜索类型参数
    initSearchType() {
      if (['wish', 'amazon', 'aliexpress', 'mercado',
        'lazada', 'allegro', 'b2w', 'kaufland', 'walmart',
        'coupang', 'linio', 'cdiscount'].includes(this.platforms[this.platform].value)) {
        this.searchTypes = [
          {
            text: '店铺序号',
            value: 'SHOP_NUMBER',
    ...
    initTable() {
      return new Promise((resolve, reject) => {
        ...
        const methods = ['wish', 'joom', 'b2w', 'mercado', 'kaufland', 'walmart', 'coupang', 'linio', 'cdiscount', 'allegro'].includes(this.platforms[this.platform].value)
          ? 'get'
          : 'post'
        const that = this
    ...
    save() {
      ...
      if (this.platforms[this.platform].value === 'wish') {
        ...
        // 使用yuanmeiShop/add yuanmeiShop/update
        ...
        //   allegro: 'allegroDialog',
        //   b2w: 'b2wDialog',
        //   mecardo: 'mecardoDialog',
        //   kuafland: 'kuaflandDialog',
        //   walmart: 'walmartDialog',
        //   coupang: 'coupangDialog',
        //   linio: 'linioDialog',
        //   cdiscount: 'cdiscountDialog'
        ...
        // allegro mecardo
        if (['allegro', 'mecardo'].includes(this.platforms[this.platform].value)) {
          if (this.shop.accessToken === '') {
            this.alertMessage('未获取令牌无法保存', 'error')
            return
          } else if (this.shop.refreshToken === '') {
        ...
        if (['kuafland', 'coupang', 'cdiscount'].includes(this.platforms[this.platform].value)) {
          const idText = this.platforms[this.platform].value === 'cdiscount' ? '请输入API账号' : '请输入ClientId'
          const secretText = this.platforms[this.platform].value === 'cdiscount' ? '请输入API密码' : '请输入Client Secret'

          if (this.shop.ClientId === '') {
            this.alertMessage(idText, 'error')
            return
          } else if (this.shop.ClientSecret === '') {
            this.alertMessage(secretText, 'error')
            return
          }
        }

        // coupang 卖家id
        if (this.platforms[this.platform].value === 'coupang') {
          const seller = this[shopObj[this.platforms[this.platform].value]].sellerNumber
        ...
        // 站点  joom allegro(没有判断)
        if (['linio', 'cdiscount', 'allegro', 'joom', 'wish'].includes(this.platforms[this.platform].value)) {
          if (this[shopObj[this.platforms[this.platform].value]].siteId === '') {
            this.alertMessage('请选择站点', 'error')
            return
          }
        }
        ...
        // 佣金比例
        if (this[shopObj[this.platforms[this.platform].value]].commissionRate === '') {
          this.alertMessage('未填写佣金比例', 'error')
          return
        }

        console.log(this.shop.ClientSecret, 'this.shop.ClientSecret')
        ...
        const obj = {
          ...
          changeDate: this.shop.time,
          platform: this.platforms[this.platform].text,
          commissionRate: this[shopObj[this.platforms[this.platform].value]].commissionRate
        }

        // id
        if (['allegro', 'joom'].includes(this.platforms[this.platform].value)) {
          obj.id = this.shop.id
        }

        // clientId
        if (this.platforms[this.platform].value === 'b2w') {
          obj.clientId = this.b2wShop.xaccountmanagerKey
        } else if (!['linio', 'b2w', 'joom'].includes(this.platforms[this.platform].value)) {
          obj.clientId = this.shop.ClientId
        }

        // clientSecret 除了b2w不需要
        if (this.platforms[this.platform].value !== 'b2w') {
          obj.clientSecret = this.shop.ClientSecret
        }

        // redirectUri 其他平台不需要
        if (['allegro', 'mercado', 'kaufland', 'joom'].includes(this.platforms[this.platform].value)) {
          obj.redirectUri = this.shop.RedirectURI
        }

        // accessToken
        if (this.platforms[this.platform].value === 'cdiscount') {
          obj.accessToken = this.cdiscountShop.token
        } else if (this.platforms[this.platform].value === 'b2w') {
          obj.accessToken = this.b2wShop.apiToken
        } else if (['allegro', 'mercado', 'joom'].includes(this.platforms[this.platform].value)) {
          obj.accessToken = this.shop.accessToken
        }

        // refreshToken expiresTime 其他平台不需要   joom的shopId
        if (['allegro', 'mercado', 'cdiscount', 'joom'].includes(this.platforms[this.platform].value)) {
          obj.refreshToken = this.shop.refreshToken
          if (this.platforms[this.platform].value === 'joom') {
            obj.expiresTime = this.shop.expiresTime
            obj.shopId = this.shop.shopId
          } else {
            obj.expiresTime = this[shopObj[this.platforms[this.platform].value]].expiresInTime
          }
        }

        // userName
        if (['b2w', 'linio'].includes(this.platforms[this.platform].value)) {
          obj.userName = this[shopObj[this.platforms[this.platform].value]].userName
        }

        // sellerId
        if (this.platforms[this.platform].value === 'mercado') {
          obj.sellerId = this.mercadoShop.sellerId
        } else if (this.platforms[this.platform].value === 'coupang') {
          obj.sellerId = this.coupangShop.sellerNumber
        }

        // siteId
        switch (this.platforms[this.platform].value) {
          case 'b2w':
            obj.siteId = 'BR'
            break
          case 'mercado':
          ...
          case 'coupang':
            ...
            break
          default:
            obj.siteId = this[shopObj[this.platforms[this.platform].value]].siteId
            break
        }
        ...
        // if (!this.regExp.test(this[shopObj[this.platforms[this.platform].value]].commissionRate)) {
        //   this.alertMessage('佣金比例为：大于0小于1的小数，请修改！', 'error')
        //   return
        // }
        // // api账号密码的验证
        ...
        // })
        // saveShopByYuanmei(requestUrl, requestMethod, obj).then(msg => {
        //   this[dialogObj[this.platforms[this.platform].value]] = false
        //   this.loading.close()
        //   this.search()
        //   this.alertMessage(msg.data, 'success')
        // }).catch(error => {
    ...
    changeWhat() {
        /* 
                ...
                1、抽离了验证判断，传参
                2、coupangShop.sellerNumber  ->  coupangShop.sellerId  待
